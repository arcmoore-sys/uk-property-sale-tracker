...
2. **Create the filing system.** Inside the user's connected Cowork folder, create
   ...
   model. This filing system is aligned with the workbook tabs and the skills, so
   every document has an obvious home:
...
7. **Set up the recurring chaser.** Create a scheduled task that runs the
   ...
   since their last NDA contact gets a polite chase. Confirm the task was created.

8. **Schedule the round-1 process letter.** Create a one-off scheduled task that
   fires `request-bids` for this deal on the agreed date.
